...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    // Initial Hero Entrance Animation (subtle slide-up; keep visible to avoid blank flash)
    gsap.from(container, { y: 40, duration: 1.4, ease: "power3.out" });

    if (textRef.current) {
      gsap.from(textRef.current, { y: -30, duration: 1.2, ease: "power3.out", delay: 0.2 });
    }
    ...
    try {
      ...
      console.warn('WebGL unavailable, skipping Hero shader:', err);
      return;
    }
    if (!renderer || !renderer.getContext()) {
      console.warn('WebGL context unavailable, skipping Hero shader.');
      return;
    }
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-black flex items-center justify-center">
      {/* Static fallback background (visible if WebGL fails to load) */}
      <div className="absolute inset-0 z-0 bg-[radial-gradient(circle_at_30%_40%,rgba(120,40,40,0.35),transparent_60%),radial-gradient(circle_at_70%_70%,rgba(40,40,120,0.25),transparent_60%)]" />
        ...
        >
          ...
          <div className="flex-1 max-w-lg lg:max-w-xl text-left">
            ...
            <h1 className="text-2xl md:text-3xl lg:text-[1rem] xl:text-[3.5rem] font-bold tracking-tighter drop-shadow-2xl leading-[1.05] font-sans">
              Engineering<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-200 to-gray-500 font-serif italic font-light pr-2">Intelligent</span> Systems<br />
              for Tomorrow
            </h1>
          </div>
          ...
          <div className=" flex-1 max-w-md text-left md:text-right flex flex-col md:items-end">
            <p className="w-110 text-lg md:text-xl text-gray-300 drop-shadow-xl font-light tracking-wide leading-relaxed mb-8">
              IT undergraduate exploring AI, Data Analytics, and Full-Stack Development. With creativity and a problem-solving mindset, I design impactful technology for a smarter future.
            </p>
            
            <a href="#contact" className="pointer-events-auto px-8 py-4 rounded-full border border-white/30 text-white text-sm tracking-[0.2em] uppercase font-medium hover:bg-white hover:text-black hover:border-white transition-all duration-500 backdrop-blur-sm shadow-xl inline-block">
              Get In Touch
            </a>
          </div>
